...
class MultiStepExampleForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    foreach ($buttons as $button) {
      $form['wrapper']['actions'][$button['#key']] = $button;
      // Add ajax to button.
      $form['wrapper']['actions'][$button['#key']]['#ajax'] = [
        'callback' => [$this, 'loadStep'],
        'wrapper' => 'form-wrapper',
        'effect' => 'fade',
      ];

      if ($button['#submit_handler']) {
        // Attach submit handler to button, so we can execute it later on..
        $form['wrapper']['actions'][$button['#key']]['#submit_handler'] = $button['#submit_handler'];
      }
   ...
   *   Form state interface.
   */
  public function emailSubmitToMaropost(array &$form, FormStateInterface $form_state) {
